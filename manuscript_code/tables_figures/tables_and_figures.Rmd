...
mids <- barplot(t(means), beside=TRUE, col='gray', border='white', 
	ylim=c(0.03, 0.06), xpd=FALSE, axes=FALSE, xaxt='n')
...
par(las=1)
axis(2, at=c(.03, .04, .05, .06), cex.axis=1, tck=.02, hadj=.5, col='gray30')
...
mids <- barplot(t(means), beside=TRUE, col='gray', border='white', 
	ylim=c(0.01, 0.06), xpd=FALSE, axes=FALSE, xaxt='n')
abline(h=seq(0.01, 0.06, 0.005), col='white')
arrows(c(t(mids)), unlist(means-ses), c(t(mids)), unlist(means+ses), 
	length=0.03, code=3, angle=90, col='gray40', lwd=2)
par(las=1)
axis(2, at=c(.01, .02, .03, .04, .05, .06), cex.axis=1, tck=.02, hadj=.5, col='gray30')
par(las=0, lheight=.8, xpd=TRUE)
